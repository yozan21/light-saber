...
var angle = -Math.PI / 2;
var active = false;
var particleCount = 195 + handleHeight;
var prevAngle = angle;
var mouse = {
  x: innerWidth / 2,
  y: innerHeight / 2
...
addEventListener("mousemove", function (e) {
  ...
  if (active && !sound.closeMvmtSound) {
    ...
    if (angDiff > 0.5 && !audio.LMov.playing()) {
      audio.LMov.play();
    } else if (angDiff > 0.05 && (!audio.MMov.playing() || !audio.LMov.playing())) {
      // Howler.stop();
      audio.MMov.play();
    }
...
addEventListener("click", function () {
  active = !active;
  if (active) audio.On.play();else audio.Off.play();
  // console.log(active);
});
  ...
  return _createClass(Particle, [{
    ...
    value: function update(on) {
      ...
      if (this.distFromCenter < handleHeight) {
        this.draw();
        this.color = this.distFromCenter > 10 && this.distFromCenter < 25 || this.distFromCenter > 40 && this.distFromCenter < 45 ? "#333333" : "#D9DADB";
        this.radius = 5;
        this.blur = 0;
      }
      ...
      this.y = center.y + handleHeight / 2 + this.distFromCenter * Math.sin(angle);
    }
  }]);
}(); // Implementation
var particles;
function init() {
  ...
  for (var i = 0; i < particleCount; i++) {
    ...
    var x = center.x + dist * Math.cos(0);
    var y = center.y + handleHeight / 2 + dist * Math.sin(0);
    particles.push(new Particle(x, y, 3, i < particleCount - 1 ? "#ffff" : "rgba(30, 144, 255, 0.25)", i));
  }
}
